...
void setup() 
{
  ...
  lcd.begin(16, 2);
  attachInterrupt(0,rise,RISING); //Digital Pin 2 corresponds to Interrupt 0, Calls ISR function 'rise' everytime Pin 2 has a rising edge
  attachInterrupt(1,fall,FALLING); //Digital Pin 3 corresponds to Interrupt 1, Calls ISR function 'fall' everytime Pin 3 has a falling edge
    ...
    for (int i = 0; i < 16; i++) {
        if (diff/1000 > pulse_width[i] - threshold & diff/1000 < pulse_width[i]+ threshold) {
            ...
            if(cursorColumn == 16) {        
                ...
                if(cursorRow == 1)
                {
                  lcd.clear(); // if reaching limit, clear LCD
                  cursorRow = 0;
                }
                else 
                {
